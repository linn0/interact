...
(defun make-carray-from-vector (vector)
  (let ((buffer (#_malloc (length vector))))
    (dotimes (index (length vector))
      (setf (paref buffer (:array :unsigned-byte) index) (aref vector index)))
    buffer))

(defun make-vector-from-carray (carray size)
  (let ((vector (make-array size :element-type '(unsigned-byte 8))))
    (dotimes (index size)
      (setf (aref vector index) (paref carray (:array :signed-byte) index)))
    vector))
