...
Note that this operation is costly, see VECTOR-PUSH-EXTEND-FRONT"
  (setf (parent child) parent)
  (ext:vector-push-extend-front child (children parent))
  child)
...
See VECTOR-PUSH-EXTEND-POSITION"
  (ext:vector-push-extend-position
   new-child
   (family element)
...
See ARRAY-SHIFT"
  (let* ((parent (parent element))
    ...
    (cond ((= 0 count)
           (ext:vector-pop-position family position)
           parent)
          (T
           (ext:array-shift (children parent) :n (1- count) :from position)
           (loop repeat count
                 for i from position
