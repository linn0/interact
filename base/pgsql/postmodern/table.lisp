...
(defclass dao-class (cl:standard-class)
...
(defmethod ccl:validate-superclass ((class dao-class) (super-class cl:standard-class))
  t)
...
(defun dao-column-fields (class)
  (mapcar 'ccl:slot-definition-name (dao-column-slots class)))
  ...
  (let ((found ()))
    (labels ((explore (class)
               (when (typep class 'dao-class)
                 (pushnew class found))
               (mapc #'explore (ccl:class-direct-superclasses class))))
      (explore class)
      found)))

(defmethod ccl:finalize-inheritance :after ((class dao-class))
...
(defclass direct-column-slot (ccl:standard-direct-slot-definition)
...
(defmethod ccl:direct-slot-definition-class ((class dao-class) &key column col-type &allow-other-keys)
...
(defclass effective-column-slot (ccl:standard-effective-slot-definition)
  ((direct-slot :initform *direct-column-slot* :reader slot-column)))

(defmethod ccl:compute-effective-slot-definition ((class dao-class) name direct-slot-definitions)
  (declare (ignore name))
...
(defmethod ccl:effective-slot-definition-class ((class dao-class) &rest initargs)
...
(defgeneric get-dao (type &rest args)
  (:method ((class-name symbol) &rest args)
    (let ((class (find-class class-name)))
      (if (ccl:class-finalized-p class)
...
values.)"
  ...
  (setf (slot-value class 'column-map)
        (mapcar (lambda (s) (cons (slot-sql-name s) (ccl:slot-definition-name s))) (dao-column-slots class)))

  (%eval
    `(let* ((fields (dao-column-fields ,class))
            (key-fields (dao-keys ,class))
            (ghost-slots (remove-if-not 'ghost (dao-column-slots ,class)))
            (ghost-fields (mapcar 'ccl:slot-definition-name ghost-slots))
       ...
       (labels ((field-sql-name (field)
         ...
         (let* ((defaulted-slots (remove-if-not (lambda (x) (slot-boundp x 'col-default))
                ...
                (defaulted-names (mapcar 'ccl:slot-definition-name defaulted-slots))
...
(defun dao-table-definition (table)
  ...
  (unless (ccl:class-finalized-p table)
    (ccl:finalize-inheritance table))
  (sql-compile
   `(:create-table ,(dao-table-name table)
                   ,(loop :for slot :in (dao-column-slots table)
                          :unless (ghost slot)
                          :collect `(,(ccl:slot-definition-name slot) :type ,(column-type slot)
