...
(defun content-type-external-format (content-type)
  ...
  (let ((charset (content-type-parameter content-type "charset")))
    (cond ((null charset) nil)
          ...
          ((string-equal charset "gbk") :cp936)
          ((string-equal charset "gb2312") :gb2312)
          ...
          (t (error "charset ~A is unrecognized." charset)))))
...
(defun content-type-decode (content-type bytes)
  ...
  (if (content-type-text-p content-type)
    (let ((format (content-type-external-format content-type)))
      (if (null format) (setq format (detect-content-encoding bytes)))
      (ccl:decode-string-from-octets bytes :external-format format))
    bytes))

(defun detect-content-encoding (bytes)
  (let ((start (search (map 'vector #'char-code "http-equiv=\"Content-Type\"") bytes)))
    (if start
      (let* ((end (position (char-code #\>) bytes :start start))
             (line (ccl:decode-string-from-octets 
                     (subseq bytes start end) 
                     :external-format :us-ascii)))
      (if (null line)
        :utf-8
        (re:with-re-match (match (re:match-re #r".*content=\"([^\"]+)\"" line))
          (content-type-external-format (content-type-parse $1)))))
      :utf-8)))
