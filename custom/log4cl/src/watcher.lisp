...
(defvar *stop-semaphore* (ccl:make-semaphore))
           ...
           (when (with-hierarchies-lock
                   (cond (*watcher-thread*
                          (%with-local-interrupts
                            ...
                            nil))
                         (t (setq *watcher-thread* (ext:current-thread)))))
             (unwind-protect
                  (%with-local-interrupts
                    (handler-case
                        (progn
                          ...
                          (loop for *watcher-event-time* = (get-universal-time)
                                ...
                                until (ccl:timed-wait-on-semaphore *stop-semaphore* *hierarchy-watcher-heartbeat*)))
                      (error (e)
