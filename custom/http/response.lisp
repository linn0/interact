...
(defun http-read-status (http)
  ...
  (let ((line (read-line http)))
    (with-re-match (match (match-re "^HTTP/(%S+)%s+(%d+)%s*(%N*)" line))
...
(defun http-read-content (http &optional content-length)
  ...
  (cond ((null content-length)
         (http-read-content-chunked http 4096))
...
(defun http-write-response (resp)
  ...
  (let ((http (resp-stream resp))
    ...
    (if (and body req (string/= (req-method req) "HEAD"))
      (progn 
        (setf (resp-body resp) (http-encode body resp))
        (http-write-headers resp http)
        (write-sequence (resp-body resp) http))
      (http-write-headers resp http))
...
(defun http-async-read-response (req http data)
  "Read a response string from an HTTP socket stream and parse it."
  (with-input-from-string (stream (decode-string-from-octets data))
    (multiple-value-bind (protocol code status)
        (http-read-status stream)
      (let* ((headers (http-read-headers stream))
        ...
        ;; return the response, but read the body first if wanted
        (if (req-read-body req)
          (http-async-read-response-body resp)
          (ext:promisify resp))))))

;;; ----------------------------------------------------

(defun http-async-read-response-body (resp)
  "Read the response body from the stream."
  (with-slots (stream body)
      resp
    ...
    (let ((length (http-header resp "Content-Length"))
      ...
      (ext:attach 
        (cond ((null encoding)
               (http-async-read-content stream length))
              ((string= encoding "identity")
               (http-async-read-content stream length))
              ((string= encoding "chunked")
               (http-async-read-content-chunked stream)))
        (lambda (body) 
          (setf (resp-body resp) body)
          ;; attempt to decode the body
          (http-decode-response-body resp)
...
(defun http-async-read-content (http &optional content-length)
  "Read the rest of the response from the HTTP server."
  (cond ((null content-length)
         (http-async-read-content-chunked http 4096))

        ;; if a string from a header, parse, and retry
        ((stringp content-length)
         (let ((n (parse-integer content-length :junk-allowed t)))
           (http-async-read-content http n)))

        ;; read the body, assuming length is ok
        (t (ext:async-read http content-length))))

;;; ----------------------------------------------------

(defun http-async-read-content-chunked (http &optional chunk-size)
  "Read the body from the HTTP server using a chunked Transfer-Encoding."
  (ext:chain 
    (if chunk-size
      (ext:promisify chunk-size)
      (ext:attach 
        (ext:async-read-until http 
          (lambda (data) 
            (let* ((delimiter (map 'vector #'char-code #(#\Return #\Linefeed)))
                   (position (search delimiter data :from-end t)))
              (if position (+ position (length delimiter))))))
        (lambda (size-string) 
          (parse-integer size-string :radix 16 :junk-allowed t))))
    (:then (size)
      (if (and size (> size 0))
        (ext:async-read http size)))
    (:then (chunk)
      (if (> (length chunk) 0)
        (ext:attach 
          (http-async-read-content-chunked http chunk-size)
          (lambda (remaining-chunk)
            (concatenate '(vector (unsigned-byte 8)) chunk remaining-chunk)))))))
