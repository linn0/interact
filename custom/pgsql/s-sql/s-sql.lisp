...
(def-sql-op :aref (form start &optional end)
  (if end
...
(def-sql-op :create-table (name (&rest columns) &rest options)
  ...
  `("CREATE TABLE IF NOT EXISTS " ,(to-sql-name name) " ("
...
(def-sql-op :create-index (name &rest args)
  (cons "CREATE INDEX IF NOT EXISTS " (expand-create-index name args)))

(def-sql-op :create-unique-index (name &rest args)
  (cons "CREATE UNIQUE INDEX IF NOT EXISTS " (expand-create-index name args)))
...
(def-sql-op :create-sequence (name &key increment min-value max-value start cache cycle)
  `("CREATE SEQUENCE IF NOT EXISTS " ,@(sql-expand name)
