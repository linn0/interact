    ...
    (handler-case
        (let ((socket (cond
                        ((equal (connection-host conn) :unix)
                         (assert-unix)
                         #+unix(unix-socket-connect (unix-socket-path *unix-socket-dir* (connection-port conn))))
                        ((and (stringp (connection-host conn))
                              (char= #\/ (aref (connection-host conn) 0)))
                         (assert-unix)
                         #+unix(unix-socket-connect (unix-socket-path (connection-host conn) (connection-port conn))))
                        ((and (pathnamep (connection-host conn))
                              (eql :absolute (pathname-directory (connection-host conn))))
                         (assert-unix)
                         #+unix(unix-socket-connect (unix-socket-path (namestring (connection-host conn))
