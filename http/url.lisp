...
(defun copy-url (uri &key
                       (scheme (url-scheme uri))
                       (domain (url-domain uri))
                       (auth (url-auth uri))
                       (port (url-port uri))
                       (path (url-path uri))
                       (query (url-query uri))
...
(defun merge-uri-paths (ref-path base-path)
  ...
  (let* ((path-list (and base-path (nreverse (ext:split #\/ base-path))))
         (ref-components (and ref-path (ext:split #\/ ref-path)))
         ending-slash-p)
...
mutated."
  ...
  (when (url-equal reference base)
    (return-from merge-uris base))

  ;; Step 3 -- scheme
  (when (url-scheme reference)
    (return-from merge-uris reference))
  (let ((uri (copy-url reference :scheme (url-scheme base))))
    (when (null (url-port uri))
      (setf (url-port uri) (url-port-lookup (url-scheme uri))))
    (macrolet ((done () '(return-from merge-uris uri)))

      ;; Step 4 -- Authority
      (when (url-domain uri)
        (done))
      (setf (url-auth uri) (url-auth base))
      (setf (url-domain uri) (url-domain base))
      (setf (url-port uri) (url-port base))

      ;; Step 5 -- Empty path
      (when (null (url-path uri))
        (setf (url-path uri) (url-path base))
        (done))

      ;; Step 6 -- Absolute path
      (ext:when-let* ((p (url-path uri))
                             (first-char (and (> (length p) 0) (char p 0))))
        (when (char= #\/ first-char)
          (done)))

      ;; Step 7 -- Relative path
      (setf (url-path uri) (merge-uri-paths (url-path uri) (url-path base)))
...
(defun url-resolve (base reference)
  (if (null reference)
    (return-from url-resolve ""))
  (let ((base-url (url-parse base))
        (ref-url (url-parse reference)))
    (with-output-to-string (stream)
      (print-object (merge-uris ref-url base-url) stream))))
